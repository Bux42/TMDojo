...
app.use(
  cors({
    origin: ['http://localhost:4200', 'https://tmdojo.com'],
    credentials: true
  })
);
...
app.get('/get-tmxid', async (req, res, next) => {
  console.log(req.query);
  let tmxId;

  try {
    const tmxRes = await axios.get(`https://trackmania.exchange/api/maps/get_map_info/multi/${req.query.mapUId}`, {
      withCredentials: true
    });

    const tmxData = tmxRes.data;
    if (tmxData[0] && tmxData[0].TrackID) {
      tmxId = tmxData[0].TrackID;
    } else {
      console.debug('No TMX data available for map:', mapId);
    }
  } catch (error) {
    console.error(error);
  }
