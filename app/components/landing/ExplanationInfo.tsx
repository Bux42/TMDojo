...
const ExplanationInfo = (): JSX.Element => {
    ...
    const whatIsThis = [
        <span className="w-full text-base text-center font-bold">
    ...
    const tabContent = {
        whatIsThis: <div>{transformTextToDOM(whatIsThis)}</div>,
        howDoIRecord: <div>{transformTextToDOM(howDoIRecord)}</div>,
        whereDoIAnalyze: <div>{transformTextToDOM(whereDoIAnalyze)}</div>,
    };
            ...
            <div className="w-full flex justify-center gap-4 md:gap-12 text-md lg:text-lg font-bold">
                {tabList.map(({ key, title }) => (
                    <div
                        ...
                        style={{
                            backgroundColor:
                                key === infoTab ? '#171717' : '#1F1F1F',
                            transition: 'all 0.15s ease-out',
                            transform:
                                key === infoTab ? 'translate(0px, 4px)' : '',
                            width: `${100 / (tabList.length + 1)}%`,
                        }}
                        className={`flex items-center justify-center py-2 px-4 rounded-md cursor-pointer select-none text-center ${
                            key === infoTab ? 'shadow' : ''
                        }`}
