...
const replaysToDataPoints = (replays_: ReplayInfo[]): ChartDataPoint[] =>
    replays_.map((replay) => ({
        x: replay.date,
...
const filterReplaysByUser = (
    loggedInUser: AuthUserInfo,
    inputReplays: ReplayInfo[],
) => {
    const filtered = inputReplays.filter(
        (r) => r.webId === loggedInUser.accountId,
    );
    return filtered;
};
...
const FastestTimeProgression = ({
    ...
    const finishedReplays = useMemo(
        () => replays.filter((r) => r.raceFinished === 1),
        [replays],
    );
    ...
    const personalTimeProgressionData: ChartDataPoint[] | undefined =
        useMemo(() => {
            if (userToShowProgression === undefined) {
                return undefined;
            }
            const filteredReplays = filterReplaysByUser(
                userToShowProgression,
                finishedReplays,
            );
    ...
    const allDataPoints: ChartDataPoint[] = useMemo(() => {
        ...
        filteredReplays = filteredReplays.filter(
            (r1) =>
                !some(
                    timeProgressionData,
                    (r2: ChartDataPoint) => r1._id === r2.replay._id,
                    null,
                ),
        );
            ...
            line: {
                dataLabels: {
                    ...
                    formatter: function dataLabelFormatter(this: any) {
                        // eslint-disable-next-line react/no-this-in-sfc
                        return `${
                            this.point.replay.playerName
                        } - ${getRaceTimeStr(this.y)}`;
                    },
                    color: 'white',
                    shadow: false,
                    padding: 0,
        ...
        <>
            <div className="flex flex-col items-center w-full mb-6">
                <div className="text-xs italic mb-4">
                    Note: All these times are from TMDojo data only. It does not
                    represent a WR progression, but a progression of the best
                    times stored on TMDojo.
                </div>
                <div>
                    <b>Fastest Time</b>
                </div>
                <div className="text-xl mb-1">
                    ...
                    {' by '}
                    <PlayerLink
                        webId={fastestTime.replay.webId}
                        name={fastestTime.replay.playerName}
                    />
                </div>
                <div className="text-xs">
                    {timeDifference(
                        new Date().getTime(),
                        fastestTime.replay.date,
                    )}
                </div>
            </div>
            <HighchartsReact
