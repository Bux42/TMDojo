...
const useUpdateReplayLineTrail = (
    ...
    const updateLineTrail = () => {
        ...
        const curSampleIndex = getSampleIndexNearTime(
            replay,
        ...
        if (showFullTrail || showTrailToStart) {
            if (
                ...
                previousLineUpdate.current.segmentUpdate
            ) {
            ...
            // Clamp trail indices
            const startTrailIndexClamped = Math.max(
                0,
                Math.min(startTrailIndex, replay.samples.length),
            );
            const endFadeIndexClamped = Math.max(
                0,
                Math.min(endFadeIndex, curSampleIndex, replay.samples.length),
            );
            ...
            if (
                ...
                for (let i = updateStart; i < updateEnd; i++) {
                    if (i < startTrailIndexClamped) {
                        ...
                        const alpha =
                            (i - startTrailIndexClamped) /
                            (endFadeIndexClamped - startTrailIndexClamped);
            ...
            bufferRef.current.setDrawRange(
                startTrailIndexClamped,
                samplesToDraw,
            );
