...
export const interpolateSamples = (
    ...
    const factor: number =
        (currentRaceTime - prev.currentRaceTime) /
        (current.currentRaceTime - prev.currentRaceTime);
    ...
    smooth.fLDamperLen = interpolateFloat(
        prev.fLDamperLen,
        current.fLDamperLen,
        factor,
    );
    smooth.fRDamperLen = interpolateFloat(
        prev.fRDamperLen,
        current.fRDamperLen,
        factor,
    );
    smooth.rLDamperLen = interpolateFloat(
        prev.rLDamperLen,
        current.rLDamperLen,
        factor,
    );
    smooth.rRDamperLen = interpolateFloat(
        prev.rRDamperLen,
        current.rRDamperLen,
        factor,
    );

    smooth.wheelAngle = interpolateFloat(
        prev.wheelAngle,
        current.wheelAngle,
        factor,
    );
...
export const getSampleIndexNearTime = (
    replay: ReplayData,
    raceTime: number,
): number => {
    ...
    while (
        sampleIndex + 1 < replay.samples.length &&
        replay.samples[sampleIndex].currentRaceTime < raceTime
    ) {
        sampleIndex++;
    }
...
export const getSampleNearTime = (
    replay: ReplayData,
    raceTime: number,
): ReplayDataPoint => {
